    ...
    <>
      ...
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="min-w-0 flex-1">
          <h3 className="text-base font-semibold text-gray-800 truncate">{account.issuer || "未知服务"}</h3>
          <p className="text-sm text-gray-500 truncate">{account.name}</p>
        </div>
        <span className="ml-2 shrink-0 px-2 py-0.5 text-xs font-medium rounded-full bg-gray-100 text-gray-600 uppercase">{account.type}</span>
        {groupName && (
          <span className="ml-1 shrink-0 px-2 py-0.5 text-xs font-medium rounded-full bg-blue-50 text-blue-600">{groupName}</span>
        )}
      </div>

      {/* 验证码区域 */}
      <button type="button" onClick={handleCopyCode} aria-label={`复制验证码 ${code}`}
        className="w-full flex items-center justify-between px-4 py-3 mb-3 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors cursor-pointer group">
        <span className="text-2xl font-mono font-bold tracking-widest text-gray-900">{formattedCode}</span>
        <div className="flex items-center gap-2">
          <div className="relative w-8 h-8" aria-label={`剩余 ${remaining} 秒`}>
            ...
            <span className={`absolute inset-0 flex items-center justify-center text-xs font-medium ${remaining <= 5 ? "text-red-500" : "text-gray-600"}`}>{remaining}</span>
          </div>
          {copied === "code" ? (
            <span className="text-xs text-green-600 font-medium">已复制</span>
          ) : (
            <svg className="w-4 h-4 text-gray-400 group-hover:text-gray-600" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" aria-hidden="true">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <button type="button" onClick={handleCopySecret} aria-label="复制密钥"
          ...
          {copied === "secret" ? <span className="text-green-600">已复制</span> : "密钥"}
        </button>
        ...
        <button type="button" onClick={handleCopyUrl} aria-label="复制 otpauth URL"
          ...
          {copied === "url" ? <span className="text-green-600">已复制</span> : "URL"}
        </button>

        {/* 详情按钮 */}
        <button type="button" onClick={() => setModalMode("detail")} aria-label="查看详情"
          className="flex items-center gap-1 px-2.5 py-1.5 text-xs text-blue-600 bg-blue-50 rounded-md hover:bg-blue-100 transition-colors">
          <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" aria-hidden="true">
        ...
        {onUpdate && (
          <button type="button" onClick={() => setModalMode("edit")} aria-label="编辑账户"
            className="flex items-center gap-1 px-2.5 py-1.5 text-xs text-amber-600 bg-amber-50 rounded-md hover:bg-amber-100 transition-colors">
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" aria-hidden="true">
        ...
        <button type="button" onClick={handleDelete} aria-label={confirmDelete ? "确认删除账户" : "删除账户"}
          className={`ml-auto flex items-center gap-1 px-2.5 py-1.5 text-xs rounded-md transition-colors ${
            confirmDelete ? "text-white bg-red-500 hover:bg-red-600" : "text-gray-400 hover:text-red-500 hover:bg-red-50"
          }`}>
          <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" aria-hidden="true">
