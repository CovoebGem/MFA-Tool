...
export default function AccountCard({ account, onDelete, groupName, selected, onToggleSelect }: AccountCardProps) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 transition-shadow hover:shadow-md">
      ...
      <div className="flex items-start justify-between mb-3">
        ...
        <span className="ml-2 shrink-0 px-2 py-0.5 text-xs font-medium rounded-full bg-gray-100 text-gray-600 uppercase">
          {account.type}
        </span>
        {groupName && (
          <span className="ml-1 shrink-0 px-2 py-0.5 text-xs font-medium rounded-full bg-blue-50 text-blue-600">
            {groupName}
          </span>
        )}
      </div>
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative w-8 h-8" aria-label={`剩余 ${remaining} 秒`}>
            ...
            <span className={`absolute inset-0 flex items-center justify-center text-xs font-medium ${remaining <= 5 ? "text-red-500" : "text-gray-600"}`}>
              {remaining}
            </span>
          </div>
          {/* 复制图标/已复制提示 */}
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          aria-label={confirmDelete ? "确认删除账户" : "删除账户"}
